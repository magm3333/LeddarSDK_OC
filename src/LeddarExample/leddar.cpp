...
void displayEchoes( const std::string &aSensorName, LeddarDevice::LdSensor *aSensor )
{
    ...
    if( lNewData )
    {
       ...
       // std::cout << "Channel\tDistance\tAmplitude" << std::endl;
        ...
        {
            std::string coma= (i==lResultEchoes->GetEchoCount()-1 ? "" : ", ");
            //std::cout << lEchoes[i].mFlags;
            std::cout << "{\"canal\":" << lEchoes[i].mChannelIndex << ", ";
            std::cout << "\"distancia\":" << std::setprecision( 3 ) << ( ( float )lEchoes[i].mDistance / ( float )lDistanceScale ) << ",";
            std::cout << "\"amplitud\":" << std::setprecision( 3 ) << ( ( float )lEchoes[i].mAmplitude / ( float )lAmplitudeScale ) <<"}"<<coma;
        }
        std::cout << "]" << std::endl;
